...
namespace ContaCorrente.Api.Controllers
{
    ...
    public class ContasController : ControllerBase
    {
        ...
        [SwaggerOperation(
            ...
            Description = "Cria uma nova conta corrente com os dados fornecidos. A conta é criada inativa por padrão.",
            OperationId = "CriarConta"
        )]
        [SwaggerResponse(201, "Conta criada com sucesso", typeof(ContaResponse))]
        [SwaggerResponse(400, "Dados inválidos fornecidos", typeof(ErrorResponse))]
        [SwaggerResponse(409, "Número de conta já existe no sistema", typeof(ErrorResponse))]
        [ProducesResponseType(typeof(ContaResponse), 201)]
        [ProducesResponseType(typeof(ErrorResponse), 400)]
        [ProducesResponseType(typeof(ErrorResponse), 409)]
        public async Task<ActionResult<ContaResponse>> CriarConta(
        ...
        )
        {
            ...
            {
                return Conflict(
                    new ErrorResponse { Error = ex.Message, Code = ErrorCodes.CONTA_EXISTENTE }
                );
            }
        ...
        public async Task<ActionResult<ContaResponse>> AtivarConta(
            string id,
        ...
        )
        {
            ...
            {
                return NotFound(
                    new ErrorResponse { Error = ex.Message, Code = "CONTA_NAO_ENCONTRADA" }
                );
            }
        }
        ...
        [ProducesResponseType(typeof(ErrorResponse), 404)]
        [ProducesResponseType(typeof(ErrorResponse), 409)]
        public async Task<ActionResult> InativarConta(
            string id,
        ...
        )
        {
            ...
            {
                return NotFound(
                    new ErrorResponse { Error = ex.Message, Code = ErrorCodes.CONTA_NAO_ENCONTRADA }
                );
            }
            ...
            {
                return Conflict(
                    new ErrorResponse { Error = ex.Message, Code = ErrorCodes.CONTA_JA_INATIVA }
                );
            }
        }
        ...
        /// <summary>
        /// Obtém dados de uma conta corrente por número
        /// </summary>
        ...
        [ProducesResponseType(typeof(ContaResponse), 200)]
        [ProducesResponseType(typeof(ErrorResponse), 404)]
        public async Task<ActionResult<ContaResponse>> ObterContaPorNumero(int numero)
        {
            try
            {
                var query = new ObterContaPorNumeroQuery(numero);
                var result = await _mediator.Send(query);
